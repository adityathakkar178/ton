...
export const getLastBlock = async (provider: NetworkProvider) => {
    // const x = provider.api();
    let ton4 = new TonClient4({ endpoint: 'https://sandbox-v4.tonhubapi.com' });
    (await ton4.getLastBlock()).last.seqno;

    // if (x instanceof TonClient) {
    //     console.log("x:",x);

    //     const masterchainInfo = await x.getMasterchainInfo();
    //     return masterchainInfo.latestSeqno;
    // }

    throw new Error(
        'Unable to retrieve the last block sequence number. The provider API is not an instance of TonClient.',
    );
};
export const getAccountLastTx = async (provider: NetworkProvider, address: Address) => {
    const x = provider.api();
    if (x instanceof TonClient4) {
        const res = await x.getAccountLite(await getLastBlock(provider), address);
...
export const waitForTransaction = async (
    ...
    do {
        ...
        const x = provider.api();
        if (x instanceof TonClient4) {
            const curState = await x.getAccountLite(lastBlock, address);
