...
import { jettonContentToCell, Transfer } from '../wrappers/Transfer';
...
export async function run(provider: NetworkProvider) {
    ...
    const wallet_code = await compile('Transfer');

    const minter = Transfer.createFromConfig({ admin, content, wallet_code }, await compile('Transfer'));
